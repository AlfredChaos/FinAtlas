...
const statusConfig: Record<string, { label: string; color: string; icon: React.ElementType }> = {
  ...
  processing: { label: '处理中', color: 'bg-blue-50 text-blue-600', icon: Clock },
...
export default function ImportCenter() {
  ...
  return (
    <div className="space-y-6">
      {/* 页面标题 */}
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">导入中心</h1>
      ...
      <div className="flex gap-6">
        ...
        <div className="w-80 flex-shrink-0 space-y-4">
          <GlassCard cornerRadius={16} padding="p-5">
            <h2 className="mb-4 font-medium text-slate-900">新建导入</h2>
            ...
            {/* 账期选择 */}
            <div className="mb-4">
              <label className="mb-1.5 block text-sm text-slate-500">账期</label>
              <input
                type="month"
            ...
            {/* 文件拖拽上传区 */}
            <div className="flex h-40 cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 transition-colors hover:border-emerald-400 hover:bg-emerald-50/50">
        ...
        <div className="flex-1">
          <GlassCard cornerRadius={16} padding="p-0">
            <div className="border-b border-slate-200 px-5 py-4">
              <h2 className="font-medium text-slate-900">导入历史</h2>
            </div>

            {/* 表头 */}
            <div className="grid grid-cols-[1fr_120px_100px_100px_140px] gap-4 border-b border-slate-200 px-5 py-2.5 text-xs font-medium text-slate-400">
              <span>文件名</span>
              <span>来源</span>
              <span>记录数</span>
              <span>状态</span>
              ...
              return (
                <div key={job.id}>
                  <button
                    ...
                    className="grid w-full grid-cols-[1fr_120px_100px_100px_140px] gap-4 items-center border-b border-slate-200 px-5 py-3 text-left transition-colors hover:bg-slate-50"
                  >
                    ...
                    <span className="text-sm text-slate-500">{job.source}</span>
                    <span className="font-mono text-sm text-slate-500">{job.records}</span>
                    ...
                    <div className="border-b border-slate-200 bg-slate-50/50 px-5 py-4">
                      <div className="grid grid-cols-4 gap-4">
                        ...
                        <div>
                          ...
                          <p className="font-mono text-sm font-medium text-red-600">{job.errors}</p>
                        </div>
                        <div>
                          <p className="text-xs text-slate-400">完成时间</p>
                          <p className="text-sm text-slate-500">{job.completedAt || '—'}</p>
                        </div>
                      </div>
